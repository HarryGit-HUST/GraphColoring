...
void GraphColoringSolver::solve(TimeLeft restMilliSec, int seed) {
	...
	for (ColorId colorNum = gc.nodeNum; restMilliSec() > 0;) {									// |
		...
		for (EdgeId e = 0; e < gc.edgeNum; ++e) {												// |
			NodeId u = gc.edges[e][0];															// |
			NodeId v = gc.edges[e][1]; // a second chance for conflicting edges.				// |
			if (output[u] == output[v]) { output[u] = rand() % colorNum; }						// |
		}																						// |
																								// |
