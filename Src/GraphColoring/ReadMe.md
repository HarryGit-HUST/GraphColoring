...
- `NPBenchmark-SDK/`: NPBenchmark 测试平台所有问题 SDK 根目录, 也是判题模块的部署目录.
  ...
  - `Src/`: 源代码.
    - `GraphColoring/`: 图着色问题的 SDK.
      - `ReadMe.md`: 本文件.
      - `Main.cpp`: (**模板文件不可修改**) 程序入口 (`main()` 函数).
      - `Util.h`: (**模板文件不可修改**) 常用工具函数库.
      - `GraphColoringTester.cpp`: (**模板文件不可修改**) 数据文件解析, 结果检查, 批量测试.
      - `GraphColoring.h`: (**模板文件不可修改**) 图着色算法的关键数据结构与接口定义.
      - `GraphColoring.cpp`: 解题者编写图着色算法的入口, 提供示例求解算法代码.
