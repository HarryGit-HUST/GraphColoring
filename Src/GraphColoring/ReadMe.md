...
算例 $i \in I$ 上算法 A 求得的最优解目标函数值为 $o(i)$.
若 $b(i)$ 为 0, 则修正目标函数值 $b(i) = b(i) + 1, o(i) = o(i) + 1$ 以避免除零错误.
则算例 $i \in I$ 上算法 A 与所有解题者的最优解目标函数值绝对差距为 $g(i) = |o(i) - b(i)|$, 相对差距为 $r(i) = g(i) / b(i)$.
另设衰减参数 $t = 1$, $e$ 为自然对数.
则算法 A 在算例 $i \in I$ 上得分为 $f(i) = 1 - e^{-t \cdot r(i)}$.
算法 A 在所有算例上的总得分为 $f = \sum_{i \in I} f(i)$.

排行榜上排序时, 若两个算法总得分持平, 则比较总计算时间, 越短排名越靠前.
